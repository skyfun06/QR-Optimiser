...
function isMajeur(dateNaissance: string): boolean {
  ...
  // 18e anniversaire = même jour/mois, année + 18. Majeur si déjà passé.
  ...
  async function handleSignup() {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        // Ces métadonnées alimentent la création de la ligne `vendeurs` côté
        // base (trigger sur auth.users). Le statut est forcé à 'en_attente'.
        data: {
          espace: 'vendeur',
          prenom,
          nom,
  ...
  return (
    <div className="w-full max-w-[440px] flex flex-col gap-6 animate-fade-up">
      <div className="w-full flex flex-col items-center gap-1 text-center">
        ...
        <p className="text-sm text-[#8c8c8c]">Deviens apporteur d&apos;affaires ScanAvis</p>
      </div>
      ...
      <div className="w-full flex flex-col gap-4 p-6 md:p-8 bg-[#171717] border border-[#292929] rounded-2xl">
        ...
        >
          {loading ? 'Création...' : 'Envoyer ma candidature'}
        </button>
