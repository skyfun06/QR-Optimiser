...
// dans TABS ci-dessous : en ajouter un plus tard = une entrée + un cas de rendu.
//
...
type Vendeur = { prenom: string | null; statut: string | null }

type TabId = 'dashboard' | 'formation'
...
function VendeurTabBar({ active, onChange }: { active: TabId; onChange: (id: TabId) => void }) {
  return (
    <nav
...
export function VendeurEspace() {
  ...
  const active: TabId = tab ?? 'dashboard'
  ...
  return (
    <>
      ...
      <VendeurTabBar active={active} onChange={setTab} />
    </>
  )
}
